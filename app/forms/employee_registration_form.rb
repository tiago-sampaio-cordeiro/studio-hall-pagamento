class EmployeeRegistrationForm
  ...
  def self.model_name
    ...
    validates :cpf, :rg, :birth_date, :gender, presence: true
    validates :salary, presence: true, if: -> { clt? }
    validates :hourly_rate, presence: true, if: -> { freelancer? }

    # Usado no `new`/`edit` para pré-popular o form com dados existentes
    ...
    # Helpers usados na view no lugar de user.admin?, user.employee&.clt? etc.
    def admin_role?
      role == "admin"
    end
